...
public class BattleUI : MonoBehaviour
{
    ...
    public IEnumerator ShowMessage(string message, float pauseOverride = -1f)
    {
        ...
        {
            yield return new WaitForSeconds(pauseOverride >= 0f ? pauseOverride : MessagePause);
            yield break;
        }
    ...
    public bool SetupSkillButtons(PokemonInstance player, Action<int> onSelected)
    {
        ...
        string[] labels = new string[SkillButtons.Length];
        bool[] enabled = new bool[SkillButtons.Length];

        for (int i = 0; i < SkillButtons.Length; i++)
        {
            if (i < player.EquippedSkills.Length &&
    ...
    public void SetupOptionButtons(string[] labels, bool[] enabledStates, Action<int> onSelected)
    {
        for (int i = 0; i < SkillButtons.Length; i++)
        {
            bool hasLabel = labels != null && i < labels.Length && !string.IsNullOrWhiteSpace(labels[i]);
            SkillButtons[i].gameObject.SetActive(hasLabel);
            SkillButtons[i].onClick.RemoveAllListeners();

            if (!hasLabel)
                continue;

            bool enabled = enabledStates == null || i >= enabledStates.Length || enabledStates[i];
            SkillButtons[i].interactable = enabled;
            StyleOptionButton(SkillButtons[i], SkillButtonTexts[i], labels[i], enabled);
    ...
    void StyleOptionButton(Button btn, TextMeshProUGUI label, string text, bool usable)
    {
        Image btnImage = btn.GetComponent<Image>();
        if (btnImage != null)
        {
            btnImage.color = usable
        ...
        if (label != null)
        {
            label.text = $"<size=120%><b>{text}</b></size>";
            label.color = usable ? Color.white : new Color(0.7f, 0.7f, 0.7f, 1f);
        }
    }
