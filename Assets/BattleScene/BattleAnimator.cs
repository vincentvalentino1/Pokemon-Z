...
public class BattleAnimator : MonoBehaviour
{
    ...
    public IEnumerator CaptureShake(bool isPlayer)
    {
        Transform sprite = GetSprite(isPlayer);
        if (sprite == null) yield break;
        ...
        bool isUI = sprite is RectTransform;
        float lateralAmount = isUI ? 18f : 0.18f;
        float bounceAmount = isUI ? 6f : 0.06f;
        float duration = 0.22f;

        Vector3[] offsets =
        {
            new Vector3(-lateralAmount, bounceAmount, 0f),
            new Vector3(lateralAmount, 0f, 0f),
            new Vector3(-lateralAmount * 0.5f, -bounceAmount, 0f),
            Vector3.zero
        };
        ...
        {
            Vector3 end = origin + offsets[i];
            float elapsed = 0f;
            while (elapsed < duration / offsets.Length)
            {
                elapsed += Time.deltaTime;
                float t = Mathf.Clamp01(elapsed / (duration / offsets.Length));
                sprite.localPosition = Vector3.Lerp(start, end, EaseOutQuad(t));
                float scalePulse = 1f + Mathf.Sin(t * Mathf.PI) * 0.045f;
                sprite.localScale = new Vector3(scalePulse, 2f - scalePulse, 1f);
                yield return null;
            }
    ...
    public IEnumerator CapturePop(bool isPlayer, bool breakout)
    {
        Transform sprite = GetSprite(isPlayer);
        if (sprite == null) yield break;
        ...
        bool isUI = sprite is RectTransform;
        float jumpAmount = isUI ? 20f : 0.2f;
        float forwardAmount = isUI ? 8f : 0.08f;
        Vector3 endOffset = breakout
            ? new Vector3(0f, jumpAmount, -forwardAmount)
            : new Vector3(0f, jumpAmount * 0.4f, 0f);
        ...
        while (elapsed < 0.12f)
        {
            ...
            sprite.localPosition = Vector3.Lerp(origin, origin + endOffset, EaseOutQuad(t));
            sprite.localScale = Vector3.Lerp(Vector3.one, new Vector3(1.12f, 0.88f, 1f), t);
            yield return null;
        }

        elapsed = 0f;
        Vector3 burstPosition = sprite.localPosition;
        Vector3 burstScale = sprite.localScale;
        while (elapsed < 0.14f)
        {
            elapsed += Time.deltaTime;
            float t = Mathf.Clamp01(elapsed / 0.14f);
            sprite.localPosition = Vector3.Lerp(burstPosition, origin, EaseInQuad(t));
            sprite.localScale = Vector3.Lerp(burstScale, Vector3.one, t);
            yield return null;
        }
